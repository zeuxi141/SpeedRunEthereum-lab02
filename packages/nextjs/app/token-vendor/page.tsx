...
function ensureCorrectFormat(address: string): EthereumAddress {
  if (address.startsWith("0x")) {
    return address as EthereumAddress;
  } else {
    return `0x${address}` as EthereumAddress;
  }
}
        ...
        {!!yourTokenBalance && (
          <div className="flex flex-col items-center space-y-4 bg-base-100 shadow-lg shadow-secondary border-8 border-secondary rounded-xl p-6 mt-8 w-full max-w-lg">
            ...
            <button
              ...
              onClick={async () => {
                try {
                  const toAddressChecked: EthereumAddress = ensureCorrectFormat(toAddress);
                  await writeYourTokenAsync({
                    functionName: "transfer",
                    args: [toAddressChecked, multiplyTo1e18(tokensToSend)],
                  });
                } catch (err) {
